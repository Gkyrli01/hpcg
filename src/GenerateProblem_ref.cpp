...
void GenerateProblem_ref(SparseMatrix &A, Vector *b, Vector *x, Vector *xexact) {
	...
	if (b != 0)
		InitializeVector(*b, localNumberOfRows,
						 localNumberOfRows + (64 - localNumberOfRows % 64) * (localNumberOfRows % 64 != 0));
	if (x != 0)
		InitializeVector(*x, localNumberOfRows,
						 localNumberOfRows + (64 - localNumberOfRows % 64) * (localNumberOfRows % 64 != 0));
	if (xexact != 0)
		InitializeVector(*xexact, localNumberOfRows, localNumberOfRows + (64 - localNumberOfRows % 64) *
																		 (localNumberOfRows % 64 != 0));
	double *bv = 0;
	double *xv = 0;
	double *xexactv = 0;
...
#pragma omp parallel for
	...
	const int elementsize = numberOfNonzerosPerRow;
	int size = localNumberOfRows * elementsize;
	matrixValues[0] = new double[size];
	for (int j = 1; j < localNumberOfRows; j++) {
		matrixValues[j] = &matrixValues[0][j * elementsize];
	}
	mtxIndL[0] = new int[size];
	for (int j = 1; j < localNumberOfRows; j++) {
		mtxIndL[j] = &mtxIndL[0][j * elementsize];
	}
	for (int l = 0; l < localNumberOfRows; ++l) {
...
//	matrixDiagonalSymgs[0] = new double[localNumberOfRows];
//	for (int j = 1; j < localNumberOfRows; j++) {
//		matrixDiagonalSymgs[j] = &matrixDiagonalSymgs[0][j * 1];
//	}
	for (int k = 0; k < localNumberOfRows; ++k) {
		matrixDiagonalSymgs[k] = matrixDiagonal[k][0];
	}

	std::cout<<"created matrix:"<<localNumberOfRows<<std::endl;
	A.matrixDiagonalSYMGS = matrixDiagonalSymgs;
