...
#pragma omp parallel for
	...
	for (local_int_t i = 0; i < localNumberOfRows; i++) {
...
#endif
			if (A.geom->rank !=
				rankIdOfColumnEntry) {// If column index is not a row index, then it comes from another processor
		...
		curNeighbor != sendList.

		end();

		++curNeighbor) {
		totalToBeSent += (curNeighbor->second).
		...
		curNeighbor != receiveList.

		end();

		++curNeighbor) {
		totalToBeReceived += (curNeighbor->second).
		...
		receiveLength[neighborCount] = receiveList[neighborId].

		size();

		sendLength[neighborCount] = sendList[neighborId].

		size(); // Get count if sends/receives
		for (set_iter i = receiveList[neighborId].begin();
		i != receiveList[neighborId].
